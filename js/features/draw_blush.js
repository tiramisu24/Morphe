const drawBlush = (pos, canvas, yOffset, color) => {
  ...
  drawLeftBlush(pos,canvas, yOffset, color);
};
const drawRightBlush = (pos, canvas, yOffset, color) => {
  ...
  ctx.closePath();
  ctx.fillStyle = color;
  ctx.filter=`blur(${3*yOffset}px)`;
  ctx.fill();
};
const drawLeftBlush = (pos, canvas, yOffset,color) => {
